...
bool Log::init(const char *file_name, int close_log, int log_buf_size, int split_lines,
    ...
    if (p == nullptr) {
        snprintf(file_full_name, 255, "%d_%02d_%02d_%s", my_tm.tm_year + 1900, my_tm.tm_mon + 1,
                 my_tm.tm_mday, file_name);
    } else {
        strcpy(log_name, p + 1);
        strncpy(dir_name, file_name, p - file_name + 1);
        snprintf(file_full_name, 255, "%s_%d_%02d_%02d_%s", dir_name, my_tm.tm_year + 1900,
                 my_tm.tm_mon + 1, my_tm.tm_mday, log_name);
    }

    m_today = my_tm.tm_mday;
    m_fp = fopen(file_full_name, "a");
    if (m_fp == nullptr) {
        return false;
    }
    return true;
...
void Log::write_log(int level, const char *format, ...) {
    ...
    int m = vsnprintf(m_buf, m_log_buf_size - n + 1, format, valst);
    m_buf[n + m] = '\n';
    m_buf[n + m + 1] = '\0';
    log_str = m_buf;
